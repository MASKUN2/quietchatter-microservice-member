...
* 역할: 사용자 계정 관리 (OAuth 로그인, 회원가입, 프로필 수정, 탈퇴) 및 JWT 토큰 발급
...
* Naver OAuth 연동 로직은 레거시의 `AuthMemberServiceImpl.java`를 참고하십시오.
* 외부 API 호출은 `adaptor/out`에 구현하고, 인터페이스(Port)를 통해 application 레이어와 통신하십시오.

### D. 데이터베이스 변경

* 스키마 변경 시 Flyway 마이그레이션 스크립트를 `src/main/resources/db/migration`에 작성하십시오.
